...
gen_mp3(audio_dir, output_dir, quran_data, voice, ..., progress_cb)
  ...
  ├─ 2. ffmpeg-python concat → temp MP3 with ID3 metadata (title + artist)
  │       Primary: ffmpeg.concat() → output with map_metadata=-1, id3v2_version=3
  │       Fallback: second ffmpeg.concat() attempt if first fails
  │       progress_cb(85%)
  └─ 3. _strip_album_art(): mutagen deletes all APIC frames in-place (no re-mux)
          progress_cb(100%)
```
...
| `DAILY_HADITH_COUNT` | 3 — hadiths sent to channel per day (0 to disable) |
| `DAILY_HADITH_HOURS` | `7,14,20` — UTC hours of each send; must have ≥ `DAILY_HADITH_COUNT` entries |
...
FFmpeg must be available system-wide (`apt install ffmpeg` or equivalent). Alternatively place `ffmpeg` and `ffprobe` in `bin/` and start the bot with `PATH=./bin:$PATH python bot.py`. No extra packages needed for scheduling (uses asyncio).
